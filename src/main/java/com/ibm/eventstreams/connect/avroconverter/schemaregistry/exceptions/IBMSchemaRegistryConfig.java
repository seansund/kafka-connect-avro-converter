...
public class IBMSchemaRegistryConfig {
     ...
     *
     */
    private static final String SSL_TRUSTSTORE_PASSWORD_CONFIG = "schema.registry.ssl.keystore.password";

    /**
     *
     */
    private static final String SASL_MECHANISM = "schema.registry.ssl.mechanism";
    ...
    public static Properties toProps(Map<String, ?> configs) {
        ...
        props.put(SaslConfigs.SASL_MECHANISM, configs.get(SASL_MECHANISM));
        // TODO dynamically construct based on SAL JAAS Mechansim, currently hardcoded to plain
        String saslJaasConfig = "org.apache.kafka.common.security.plain.PlainLoginModule required "
                + "username=\"" +  configs.get(SASL_JAAS_USERNAME) + "\" password=\"" + configs.get(SASL_JAAS_PASSWORD) + "\";";
